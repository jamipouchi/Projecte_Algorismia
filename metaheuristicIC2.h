...
Conjunt reproduce(Graph &graph, float diffusionProbability, Conjunt &A, Conjunt &B)
{
    sort(A.begin(), A.end());
    sort(B.begin(), B.end());
    Conjunt child;
    ...
    while (independentCascade(graph, child, diffusionProbability) < graph.size())
    {
...
Conjunt metaheuristic(Graph &graph, float difussionProbability)
{
    vector<Conjunt> population;
    for (int i = 0; i < 10; i++)
    {
        population.push_back(random_greedy_inverse(graph, difussionProbability));
    }
    ...
    while (float(past_indicator) / float(curr_indicator) > 1.05)
    {
        ...
        for (int i = 0; i < parents.size(); i++)
        {
            ...
            {
                childs.push_back(reproduce(graph, difussionProbability, population[parents[i]], population[parents[j]]));
            }
        }
